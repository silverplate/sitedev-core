...
abstract class RobotsSitemap extends Common
{
    ...
    public function execute()
    {
        ...
        $type = \App\Cms\Front\Navigation::load('robots-sitemap', 'name');

        if ($type && $type->isPublished) {
            $documents = \App\Cms\Front\Navigation::getDocuments($type->name);
            $controllers = array();
            $sitemap = array();
            $sitemapXml = '';
            ...
            foreach ($documents as $doc) {
                ...
                if ($doc->frontControllerId) {
                    if (!isset($controllers[$doc->frontControllerId])) {
                        $controllers[$doc->frontControllerId] =
                            $doc->getController();
                    }
